...
readBAM <- function(filepath) {
  bam.gr <- as(readBamGappedAlignments(filepath), "GRanges")
  bam.gr
}

readBAM_pe <- function(filepath) {
  bam <- readBamGappedAlignmentPairs(filepath)
  
  read_starts <- pmin(start(first(bam)), start(last(bam)))
  read_ends   <- pmax(end(first(bam)), end(last(bam)))
  
  bam.gr <- GRanges(seqnames=seqnames(first(bam)), ranges=IRanges(start=read_starts, end=read_ends), strand="*")
  seqlengths(bam.gr) <- seqlengths(bam)
  bam.gr
}
...
artifact_filter <- function(bam.gr, dup_count_limit=10, ext_length) {
  ...
  for(chromosome in unique(reads$chr)) {
    ...
    #save(dup.targets, file=paste(chromosome, ".dup.targets.RData", sep=""))
    ...
    #save(kept.dups, file=paste(chromosome, ".kept.dups.RData", sep=""))
...
counts_for_ranges <- function(df, col.start, col.end, sample.ranges) {
  ...
  overlaps.table <- as.data.frame(table(overlaps$query))
